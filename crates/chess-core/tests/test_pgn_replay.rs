...
fn replay_fischer_spassky_1972_game6() {
    ...
    let game = from_pgn(pgn).expect("应成功解析 Fischer-Spassky 1972 Game 6");
...
fn replay_kasparov_topalov_1999() {
    ...
    let game = from_pgn(pgn).expect("应成功解析 Kasparov-Topalov 1999");
...
fn pgn_round_trip() {
    let pgn = include_str!("resources/famous_games/fischer-spassky-1972-game6.pgn");
    let game1 = from_pgn(pgn).unwrap();
    let exported = chess_core::to_pgn(&game1);
    let game2 = from_pgn(&exported).unwrap();

    assert_eq!(game1.history().len(), game2.history().len());
    assert_eq!(game1.result(), game2.result());

    // 验证每步走法相同
    for (i, ((mv1, _), (mv2, _))) in game1
        .history()
        .iter()
        .zip(game2.history().iter())
        .enumerate()
    {
        assert_eq!(
            mv1, mv2,
